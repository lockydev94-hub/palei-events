...
export function Footer() {
  // Render the marketing footer on every public route, but hide it
  // inside the admin app — the admin shell has its own chrome. The
  // Server Component can't read the pathname, so we wrap the markup
  // in a tiny Client Component that checks it.
  return <FooterOnPublicRoutes />;
}

// Tiny client guard. Returns null on /admin/*, otherwise renders the
// marketing footer markup.
function FooterOnPublicRoutes() {
...
function FooterMarkup() {
  // Live values from Firestore (projects/current) — falls back to the
  // static defaults from data/site.ts while loading or if Firestore is
  // unreachable, so the footer never renders blank.
  const site = useSiteSettings();
  const phone = site.phone || "";
  const email = site.email || "";
  const address = site.address || "";
  const tagline = site.tagline || "";
  const supporting = site.supporting || "";
